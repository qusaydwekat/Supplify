...
const INVOICE_STATUSES: readonly InvoiceStatus[] = ['issued', 'paid', 'partial', 'overdue']

export function SupplierInvoiceFilters() {
  const t = useTranslations('SupplierInvoiceFilters')
  ...
  const statusOptions = useMemo(
    () => [{ value: '', label: t('statusAll') }, ...INVOICE_STATUSES.map((s) => ({ value: s, label: tStatus(s) }))],
    [t, tStatus],
  )
  ...
  return (
    <div className="flex flex-wrap items-end gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <div>
        <label htmlFor="inv-status" className="mb-1 block text-xs font-medium uppercase text-slate-500">
          {t('status')}
        </label>
        <select
          id="inv-status"
      ...
      <div className="min-w-[200px] flex-1">
        <label htmlFor="inv-q" className="mb-1 block text-xs font-medium uppercase text-slate-500">
          {t('search')}
        </label>
        <input
          id="inv-q"
      ...
      <div>
        <label htmlFor="inv-from" className="mb-1 block text-xs font-medium uppercase text-slate-500">
          {t('from')}
        </label>
        <input
          id="inv-from"
      ...
      <div>
        <label htmlFor="inv-to" className="mb-1 block text-xs font-medium uppercase text-slate-500">
          {t('to')}
        </label>
        <input
          id="inv-to"
