-- Notify supplier when a variation crosses into low stock (was above threshold, now at or below 2× min order qty)
...
drop trigger if exists on_variation_low_stock on public.product_variations;

create trigger on_variation_low_stock
  after insert or update of stock_quantity on public.product_variations
  for each row
  execute function public.notify_low_stock_on_variation();
